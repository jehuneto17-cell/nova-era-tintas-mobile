...
function AguardandoConfirmacaoContent() {
  ...
  useEffect(() => {
    ...
    const recusado =
      ...
      ultima?.observacao?.startsWith("Comprovante recusado:");
    if (recusado) router.replace(`/comprovante/recusado?id=${pedido.id}`);
  }, [pedido, router]);
  ...
  if (!pedidoId || pedido === null) {
    return (
      <div className="absolute inset-0 bg-white flex flex-col items-center justify-center gap-3 px-8 text-center">
        ...
        <Link
          href="/pedidos"
          className="mt-2 h-11 px-5 border-0 rounded-2xl bg-ne-green text-white cursor-pointer flex items-center"
  ...
  const emAnalise = pedido.estado === "aguardando_confirmacao";
    ...
    <div className="absolute inset-0 bg-[#F8F8F8]">
      <div className="absolute top-0 left-0 right-0 z-30 box-border px-3 pt-[46px] pb-3 bg-white border-b border-[#EDEFED] text-center">
        <div style={{ fontFamily: "var(--font-archivo)", fontWeight: 700, fontSize: 19, letterSpacing: "-0.01em", color: "#012418" }}>
          {emAnalise ? "Aguardando Confirmação" : "Status do Pedido"}
        </div>
      </div>
        ...
        <div className="p-4 flex flex-col gap-2">
          ...
          >
            {emAnalise ? (
              ...
              <>
                <span
                  className="px-3 py-1.5 rounded-full mb-1.5"
                  style={{ background: meta.bg, color: meta.fg, fontFamily: "var(--font-archivo)", fontWeight: 700, fontSize: 13 }}
                >
                  {meta.label}
                </span>
                <span style={{ fontFamily: "var(--font-archivo)", fontWeight: 700, fontSize: 16, color: "#012418" }}>
          ...
          <div className="mt-2 bg-white border border-[#E5E5E5] rounded-xl p-4 shadow-[0_1px_8px_rgba(0,0,0,.05)] flex flex-col gap-2">
            ...
            <span className="text-xs font-bold" style={{ color: meta.bg }}>{meta.label}</span>
          </div>
          ...
          {pedido.comprovanteUrl && (
            <div className="mt-4 bg-white border border-[#E5E5E5] rounded-xl p-4 shadow-[0_1px_8px_rgba(0,0,0,.05)]">
              ...
              <div className="w-[200px] max-w-full h-[150px] mx-auto rounded-lg border border-[#E5E5E5] overflow-hidden bg-[#F1F3F1]">
                ...
                <img src={pedido.comprovanteUrl} alt="Comprovante PIX" className="w-full h-full object-cover" />
              </div>
            </div>
          )}
          ...
          {!emAnalise && (
            <button
              ...
              className="mt-4 w-full h-12 border-0 rounded-xl bg-ne-green text-white cursor-pointer flex items-center justify-center"
              style={{ fontFamily: "var(--font-archivo)", fontWeight: 700, fontSize: 14 }}
            >
